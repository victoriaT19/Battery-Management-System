...
void adBms6830_infinte_loop(uint8_t tIC, cell_asic *ic);
...
const float OV_THRESHOLD = 1.8;                 /* Volt */
const float UV_THRESHOLD = 1.2;                 /* Volt */
...
const uint32_t LOOP_MEASUREMENT_COUNT = 3;      /* Loop measurment count */
...
LOOP_MEASURMENT MEASURE_AVG_CELL        = DISABLED;        /*   This is ENABLED or DISABLED       */ // ENABLED
LOOP_MEASURMENT MEASURE_F_CELL          = DISABLED;        /*   This is ENABLED or DISABLED       */ // ENABLED
LOOP_MEASURMENT MEASURE_S_VOLTAGE       = DISABLED;        /*   This is ENABLED or DISABLED       */ // ENABLED
...
void run_command(int cmd)
{
  ...
  case 21: // faz a conversão adc e lê o valor das células (comandos 3 e 4)
    adBms6830_start_adc_cell_voltage_measurment(TOTAL_IC);
    adBms6830_read_cell_voltages(TOTAL_IC, &IC[0]);
    break;

  case 22: // comandos 3, 4 e 15
    adBms6830_start_adc_cell_voltage_measurment(TOTAL_IC);
    adBms6830_read_cell_voltages(TOTAL_IC, &IC[0]);
    adBms6830_read_status_registers(TOTAL_IC, &IC[0]);
    break;

  case 23:  // Loop infinito de medições
    adBms6830_infinte_loop(TOTAL_IC, &IC[0]);
    break;

  case 24:  // teste do SLEEP
    adBms6830_test_SLEEP(TOTAL_IC, &IC[0]);
    break;
...
void measurement_loop()
{
  ...
  {
    adBmsWakeupIc(TOTAL_IC); // não era assim, mas funciona
...
void adBms6830_infinte_loop(uint8_t tIC, cell_asic *ic)
{
  ...
  while(count < LOOP_MEASUREMENT_COUNT) {
    ...
    for (uint8_t i = 0; i < 16; i++) {
      if (IC[0].statd.c_uv[i] == 1) {
          printf("\n\tFalha de subtensao!\n");
          return;
      }
      if (IC[0].statd.c_ov[i] == 1) {
          printf("\n\tFalha de sobretensao!\n");
          return;
      }
    }
...
void adBms6830_test_SLEEP(uint8_t tIC, cell_asic *ic)
{
 // adBmsWakeupIc(tIC);
  ...
  adBmsWriteData(tIC, &ic[0], CLRFC, Clrflag, C); // estudar aqui
  adBms6830_read_status_registers(tIC, &ic[0]);
  
  // adBms6830_infinte_loop(tIC, &ic[0]);
  // adBms6830_read_status_registers(tIC, &ic[0]);
  // ThisThread::sleep_for(chrono::milliseconds(500));
  // adBms6830_read_status_registers(tIC, &ic[0]);
