...
/**
*******************************************************************************
* Function: spiSendCLRFLAG
* @brief Send clear flag command in spi line with its additional 6 registers
*
* @details This function send bms command in spi line
*      
* Parameters:
* @param [in]   tx_cmd  Tx command bytes
* @param [in]   st_reg  Registers
*
* @return None
*
*******************************************************************************
...
void spiSendCLRFLAG(uint8_t tx_cmd[2], uint8_t st_reg[6])
{
  ...
  printf("!-Armazenando o comando CLRFLAG: 0x0717-!");
  cmd[0] = tx_cmd[0];
  cmd[1] = tx_cmd[1];
  printf("\t\ncmd[0]: 0x%X", cmd[0]);
  printf("\t\ncmd[1]: 0x%X\n", cmd[1]);

  printf("!-Armazenando quais bits dos registradores serao limpos-!");
  cmd[2] = st_reg[0];
  cmd[3] = st_reg[1];
  cmd[4] = st_reg[2];
  cmd[5] = st_reg[3];
  ...
  printf("!-Calculo do PEC-!");
  cmd_pec = Pec15_Calc(8, cmd);
  cmd[8] = (uint8_t)(cmd_pec >> 8);
  cmd[9] = (uint8_t)(cmd_pec);
  printf("\t\ncmd[8]: 0x%X", cmd[8]);
  printf("\t\ncmd[9]: 0x%X\n", cmd[9]);

  adBmsCsLow();
  spiWriteBytes(10, &cmd[0]);
  adBmsCsHigh();
}
/**
*******************************************************************************
